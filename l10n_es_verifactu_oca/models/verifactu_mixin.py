    ...
    def resolve(self, system_url, public_id, context):
        if system_url and system_url.endswith(VERIFACTU_DSIG_FILE):
            return self.resolve_filename(
                file_path(f"l10n_es_verifactu_oca/data/{VERIFACTU_DSIG_FILE}"),
                context,
            )
        return None
...
class VerifactuMixin(models.AbstractModel):
    ...
    verifactu_original_document_number = fields.Char(
        ...
        copy=False,
        help=(
            "Number of the original invoice being rectified when that invoice "
            "is not registered in Odoo."
        ),
    )
    verifactu_original_document_date = fields.Date(
        ...
        copy=False,
        help=(
            "Issue date of the original invoice being rectified when that "
            "invoice is not registered in Odoo."
        ),
    )
    ...
    def _get_verifactu_schema_tree(self):
        ...
        return etree.parse(
            file_path(f"l10n_es_verifactu_oca/data/{VERIFACTU_SCHEMA_FILE}"),
            parser,
        )
